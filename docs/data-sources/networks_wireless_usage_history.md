...
page_title: "meraki_networks_wireless_usage_history Data Source - terraform-provider-meraki"
subcategory: "wireless"
description: |-
  
---

# meraki_networks_wireless_usage_history (Data Source)
...
- `client_id` (String) clientId query parameter. Filter results by network client to return per-device AP usage over time inner joined by the queried client's connection history.
- `device_serial` (String) deviceSerial query parameter. Filter results by device. Requires :band.
...
- `items` (Attributes List) Array of ResponseWirelessGetNetworkWirelessUsageHistory (see [below for nested schema](#nestedatt--items))
...
- `received_kbps` (Number) Received kilobytes-per-second
- `sent_kbps` (Number) Sent kilobytes-per-second
- `start_ts` (String) The start time of the query range
- `total_kbps` (Number) Total usage in kilobytes-per-second
