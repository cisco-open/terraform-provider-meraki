	...
	resp.Schema = schema.Schema{
		Attributes: map[string]schema.Attribute{
			...
			"uplink": schema.StringAttribute{
				MarkdownDescription: `uplink query parameter. Optional filter for a specific WAN uplink. Valid uplinks are wan1, wan2, cellular. Default will return all uplinks.`,
				Optional:            true,
			},
	...
	for _, item := range *response {
		itemState := ResponseItemOrganizationsGetOrganizationDevicesUplinksLossAndLatency{
			...
			TimeSeries: func() *[]ResponseItemOrganizationsGetOrganizationDevicesUplinksLossAndLatencyTimeSeries {
				...
				return &[]ResponseItemOrganizationsGetOrganizationDevicesUplinksLossAndLatencyTimeSeries{}
			}(),
